...
def postprocess_qa_predictions(examples, features, predictions,
    ...
    for example_index in tqdm(range(len(examples['id']))):
        ...
        for feature_index in feature_indices:
            ...
            seq_ids = features['input_ids'][feature_index]
...
def _replace_mlm_inputs(inputs_id, candidate_pred_positions, num_mlm_preds, vocab):
    # Make a new copy of tokens for the input of a masked language model,
    # where the input may contain replaced '<mask>' or random tokens
    mlm_inputs_id = [input_id for input_id in inputs_id]
    ...
    for mlm_pred_position in candidate_pred_positions:
        ...
        if random.random() < 0.8:
            ...
            if random.random() < 0.5:
                masked_id = inputs_id[mlm_pred_position]
            # 10% of the time: replace the word with a random word
            else:
                masked_id = random.randint(0, len(vocab) - 1)
        mlm_inputs_id[mlm_pred_position] = masked_id
        pred_positions_and_labels.append(
            (mlm_pred_position, inputs_id[mlm_pred_position]))
    return mlm_inputs_id, pred_positions_and_labels

def get_mlm_data_from_inputs(inputs_id, tokenizer):
  
  inputs_masked = []

  for seq in inputs_id:
    candidate_pred_position = []
    for i, input_id in enumerate(seq):
      # Special tokens are not predicted in the masked language modeling
      if input_id in [101, 102, 0]:
        continue
      candidate_pred_position.append(i)
    # 15% of random tokens are predicted in the masked language modeling task
    num_mlm_preds = max(1, round(len(candidate_pred_position) * 0.15))
    mlm_input_id, pred_positions_and_labels = _replace_mlm_inputs(seq, candidate_pred_position, num_mlm_preds, tokenizer.vocab)
    inputs_masked.append(mlm_input_id)
